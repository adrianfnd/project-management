                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body px-0 pt-0 pb-2">
                        <div class="d-flex justify-content-between align-items-center px-4 pb-3">
                            <div></div>
                            <a href="{{ route('vendor.create') }}" class="btn btn-sm btn-primary" role="button"
                                data-bs-toggle="tooltip" data-bs-placement="top" title="Create vendor">
                                <i class="fa fa-plus" style="margin-right: 5px; vertical-align: middle;"></i>
                                <span style="vertical-align: middle;">Create Vendor</span>
                            </a>
                        </div>
                                ...
                                <tbody>
                                    @foreach ($vendors as $index => $vendor)
                                        <tr>
                                            ...
                                            <td>
                                                <p class="text-sm font-weight-bold mb-0">{{ $vendor->contact_person }}</p>
                                            </td>
                                            <td class="align-middle text-center text-sm">
                                                <span
                                                    class="text-secondary text-xs font-weight-bold">{{ $vendor->phone }}</span>
                                            </td>
                                            <td class="align-middle text-center text-sm">
                                                <span
                                                    class="text-secondary text-xs font-weight-bold">{{ $vendor->email }}</span>
                                            </td>
                                            <td class="align-middle text-center">
                                                <a href="{{ route('vendor.edit', $vendor->id) }}"
                                                    class="btn btn-xs btn-primary btn-sm" role="button"
                                                    data-bs-toggle="tooltip" data-bs-placement="top" title="Edit vendor">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <button type="submit" class="btn btn-xs btn-danger btn-sm"
                                                    onclick="confirmDelete({{ $vendor->id }})" data-bs-toggle="tooltip"
                                                    data-bs-placement="top" title="Delete vendor">
